...
function StepIndicator({ current }: { current: number }) {
  return (
    <nav aria-label="Progreso de configuración" className="flex items-center gap-2 justify-center mb-8">
      <ol className="flex items-center gap-2">
        {STEPS.map((label, i) => (
          <li key={i} className="flex items-center gap-2">
            <div
              aria-label={`${label}${i < current ? " (completado)" : i === current ? " (paso actual)" : ""}`}
            ...
            >
              {i < current ? <CheckIcon size={13} aria-hidden="true" /> : <span aria-hidden="true">{i + 1}</span>}
            </div>
            {i < STEPS.length - 1 && (
              <div className={`h-0.5 w-8 transition-all ${i < current ? "bg-emerald-600" : "bg-slate-800"}`} aria-hidden="true" />
            )}
          </li>
        ))}
      </ol>
      ...
      <div className="w-full max-w-lg">
        ...
        {step === 1 && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8">
            ...
            <form onSubmit={handleProfileSubmit} className="space-y-5" noValidate>
              <div>
                <label htmlFor="onboard-name" className="text-xs text-slate-500 block mb-1.5">Nombre completo</label>
                <input
                  id="onboard-name"
                  type="text"
                  value={name}
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-blue-600 focus-visible:ring-2 focus-visible:ring-blue-500 transition-colors"
                />
              </div>
              <div>
                <label htmlFor="onboard-location" className="text-xs text-slate-500 block mb-1.5">Ubicación</label>
                <input
                  id="onboard-location"
                  type="text"
                  value={location}
              ...
              <fieldset>
                <legend className="text-xs text-slate-500 block mb-2">
                  Roles objetivo{" "}
                  <span className="text-slate-600 font-normal">(seleccioná uno o más)</span>
                </legend>
                <div className="flex flex-wrap gap-2" role="group" aria-label="Roles objetivo">
                  {ROLE_OPTIONS.map(({ value, label }) => (
                    <button
                      ...
                      className={`text-xs px-3 py-1.5 rounded-full border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${
              ...
              <div className="flex gap-3 pt-1">
                <button
                  type="button"
                  onClick={() => setStep(2)}
                  className="text-sm text-slate-400 hover:text-white transition-colors px-4 py-2.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded-lg"
                >
                  Saltar
                </button>
                <button
                  type="submit"
                  className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 px-6 py-2.5 rounded-xl font-semibold transition-colors text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
                >
                  Continuar <ArrowRightIcon size={15} aria-hidden="true" />
                </button>
              </div>
            </form>
          </div>
        ...
        {step === 2 && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8">
            ...
            <form onSubmit={handleSourceSubmit} className="space-y-4" noValidate>
              <fieldset>
                <legend className="text-xs text-slate-500 block mb-2">Tipo de contenido</legend>
                <div role="group" aria-label="Tipo de contenido" className="flex gap-2">
                  {SOURCE_TYPES.map(({ value, label, icon: Icon }) => (
                    <button
                      ...
                      className={`flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${
                        sourceType === value
              ...
              <div>
                <label htmlFor="onboard-source-text" className="text-xs text-slate-500 block mb-1.5">Contenido</label>
                <textarea
                  id="onboard-source-text"
                  value={rawText}
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-blue-600 focus-visible:ring-2 focus-visible:ring-blue-500 resize-none transition-colors"
                />
                <p className="text-xs text-slate-600 mt-1 text-right" aria-live="polite">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed px-6 py-2.5 rounded-xl font-semibold transition-colors text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
                >
                  Analizar con IA <ZapIcon size={15} aria-hidden="true" />
                </button>
              </div>
            </form>
          </div>
        ...
        {/* Step 3 — Processing */}
        {step === 3 && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center" role="status" aria-label="Analizando tu perfil">
            <RefreshCwIcon
              size={40}
              aria-hidden="true"
            ...
            <ul className="space-y-2 text-left" aria-label="Pasos en progreso">
              {[
                ...
                <li key={i} className="flex items-center gap-3 text-sm text-slate-500">
                  <div className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse" style={{ animationDelay: `${i * 200}ms` }} aria-hidden="true" />
                  {stepLabel}
                </li>
              ))}
            </ul>
